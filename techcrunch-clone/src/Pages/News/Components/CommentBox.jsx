...
import {Card, CardHeader, Avatar, IconButton, Menu, MenuItem} from '@material-ui/core'
...
export const CommentBox = ({data}) =>{
    ...
    const openEditBox = (id) =>{
        let comment = data.comments.find(comment =>String(comment.id) === String(id))
    ...
    const handleEditComment = (commentId) =>{
       console.log("edit commit func called");
        ...
        let afterEditComment = data.comments.map(item => String(item.id) === String(commentId)
    ...
    const handleDeleteComment = (commentId) =>{
        ...
        // console.log(data.comments);
        // console.log("commentId",commentId);
        let afterDeleteComment = data.comments.filter(item=> String(item.id) !== String(commentId))
                ...
                data.comments && data.comments.map(item=>(
                    <Card key={item.id} className={classes.card}>
                        <CardHeader
                          ...
                          action={
                            <IconButton aria-label="settings" onClick={handleOpenAnchorEl}>
                              <MoreVertIcon />
                            </IconButton>
                          }
                          title={item.username}
                        ...
                        >
                          ...
                          <MenuItem onClick={()=>handleDeleteComment(item.id)}>Delete</MenuItem>
                        </Menu>
                         <EditCommentBox id={item.id} commentText={commentText} open={openEditComment} handleChange={handleChange} handleClose={closeEditBox} handleEditComment={handleEditComment}/>
